<!doctype html>
{{--<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">--}}
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">

    <!-- CSRF Token -->
    <meta name="csrf-token" content="{{ csrf_token() }}">

    <title>{{ config('app.name', 'Laravel') }}</title>

    <!-- Scripts -->
    <script src="{{ asset('js/app.js') }}" defer></script>

    <!-- Fonts -->
    <link rel="dns-prefetch" href="//fonts.gstatic.com">
    <link href="https://fonts.googleapis.com/css?family=Nunito" rel="stylesheet">

    <!-- Styles -->
    <link href="{{ asset('css/app.css') }}" rel="stylesheet">
</head>
<body>
<div id="app">
    <div class="p-3 pt-3 pb-0 blue-header container-fluid position-fixed top-0">
        <div class = "row justify-content-between p-4 pb-0 pt-0">
            <div class = "col-lg-4 col-md-4 col-xl-4 col-sm-3 col-auto">
                <svg style = "cursor: pointer" class = "mt-4" width="30" height="24" viewBox="0 0 30 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M1 7H15" stroke="#25254F" stroke-width="2" stroke-linecap="round"/>
                    <path d="M1 12H15" stroke="#25254F" stroke-width="2" stroke-linecap="round"/>
                    <path d="M1 17H15" stroke="#25254F" stroke-width="2" stroke-linecap="round"/>
                </svg>

            </div>
            <div class = "col-lg-4 col-md-4 col-xl-4 col-sm-6 col-auto">
                <div class = "text-center">
                    <img src="{{asset('images/ultralite-logo.svg')}}">
                </div>
            </div>
            <div class = "col-lg-4 col-md-4 col-xl-4 col-sm-3 text-end col-auto">
                <svg style = "cursor: pointer" class = "mt-4" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M7.75 4.25C7.75 1.90279 9.65279 0 12 0C14.3472 0 16.25 1.90279 16.25 4.25C16.25 6.59721 14.3472 8.5 12 8.5C9.65279 8.5 7.75 6.59721 7.75 4.25ZM12 1.5C10.4812 1.5 9.25 2.73122 9.25 4.25C9.25 5.76878 10.4812 7 12 7C13.5188 7 14.75 5.76878 14.75 4.25C14.75 2.73122 13.5188 1.5 12 1.5Z" fill="#25254F"/>
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M8 11.5C6.75736 11.5 5.75 12.5074 5.75 13.75V14.9383C5.75 14.9564 5.76311 14.9718 5.78097 14.9747C9.89972 15.6472 14.1003 15.6472 18.219 14.9747C18.2369 14.9718 18.25 14.9564 18.25 14.9383V13.75C18.25 12.5074 17.2426 11.5 16 11.5H15.6591C15.6328 11.5 15.6066 11.5042 15.5815 11.5123L14.716 11.795C12.9512 12.3712 11.0488 12.3712 9.28398 11.795L8.41847 11.5123C8.39342 11.5042 8.36722 11.5 8.34087 11.5H8ZM4.25 13.75C4.25 11.6789 5.92893 10 8 10H8.34087C8.52536 10 8.70869 10.0292 8.88407 10.0864L9.74959 10.3691C11.2119 10.8465 12.7881 10.8465 14.2504 10.3691L15.1159 10.0864C15.2913 10.0292 15.4746 10 15.6591 10H16C18.0711 10 19.75 11.6789 19.75 13.75V14.9383C19.75 15.6915 19.2041 16.3337 18.4607 16.4551C14.1819 17.1537 9.8181 17.1537 5.53927 16.4551C4.79588 16.3337 4.25 15.6915 4.25 14.9383V13.75Z" fill="#25254F"/>
                </svg>

            </div>

        </div>
        <div class = "row mt-3">
            <div class = "col-lg-8 col-md-8 col-xl-8 col-sm-12 m-auto ">
                <ul class = "flex list-unstyled">
                    <a href="#" class = "text-decoration-none text-dark m-auto">
                        <li> PRODUCTS <svg width="12" height="7" viewBox="0 0 12 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M11 0.999999L6 6L1 1" stroke="#25254F" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                        </li>
                    </a>
                    <a href="#" class = "text-decoration-none text-dark m-auto">
                        <li> PROGRAM <svg width="12" height="7" viewBox="0 0 12 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M11 0.999999L6 6L1 1" stroke="#25254F" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                        </li>
                    </a>
                    <a href="#" class = "text-decoration-none text-dark m-auto">
                        <li> PRACTITIONER </li>
                    </a>
                    <a href="#" class = "text-decoration-none text-dark m-auto">
                        <li> ABOUS AS </li>
                    </a>
                    <a href="#" class = "text-decoration-none text-dark m-auto">
                        <li>  BLOG </li>
                    </a>
                </ul>
            </div>
        </div>

    </div>

    <main class="">
        @yield('content')
    </main>
    <div class = "footer">
        <!-- Footer -->
        <footer class="bg-light text-center ">
            <!-- Grid container -->
            <div>
                <!-- Section: Social media -->
                <section>
                    <div class = "m-auto footer-social d-flex justify-content-around p-5 pt-3 pb-3" >
                    <!-- Instagram -->
                        <a class = "text-decoration-none" href="#">
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M14.0002 5.00005C14.0002 4.44776 14.4479 4.00005 15.0002 4.00005C15.5525 4.00005 16.0002 4.44776 16.0002 5.00005C16.0002 5.55233 15.5525 6.00005 15.0002 6.00005C14.4479 6.00005 14.0002 5.55233 14.0002 5.00005Z" fill="#FBF8EF"/>
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M10.0002 5.25005C7.37683 5.25005 5.25018 7.37669 5.25018 10C5.25018 12.6234 7.37683 14.75 10.0002 14.75C12.6235 14.75 14.7502 12.6234 14.7502 10C14.7502 7.37669 12.6235 5.25005 10.0002 5.25005ZM6.75018 10C6.75018 8.20512 8.20526 6.75005 10.0002 6.75005C11.7951 6.75005 13.2502 8.20512 13.2502 10C13.2502 11.795 11.7951 13.25 10.0002 13.25C8.20526 13.25 6.75018 11.795 6.75018 10Z" fill="#FBF8EF"/>
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M15.2584 0.833061C11.7919 0.44562 8.20853 0.44562 4.74195 0.833061C2.7299 1.05794 1.10556 2.64295 0.869009 4.66548C0.454471 8.20978 0.454471 11.7903 0.869009 15.3346C1.10556 17.3571 2.7299 18.9422 4.74195 19.167C8.20853 19.5545 11.7919 19.5545 15.2584 19.167C17.2705 18.9422 18.8948 17.3571 19.1314 15.3346C19.5459 11.7903 19.5459 8.20978 19.1314 4.66548C18.8948 2.64295 17.2705 1.05794 15.2584 0.833061ZM4.90856 2.32378C8.26441 1.94871 11.736 1.94871 15.0918 2.32378C16.4219 2.47244 17.4874 3.52205 17.6415 4.83973C18.0425 8.26825 18.0425 11.7318 17.6415 15.1604C17.4874 16.478 16.4219 17.5277 15.0918 17.6763C11.736 18.0514 8.26441 18.0514 4.90856 17.6763C3.57846 17.5277 2.51297 16.478 2.35885 15.1604C1.95786 11.7318 1.95786 8.26825 2.35885 4.83973C2.51297 3.52205 3.57845 2.47244 4.90856 2.32378Z" fill="#FBF8EF"/>
                            </svg>
                        </a>
                        <!-- Youtube -->
                        <a class = "text-decoration-none" href="#">
                            <svg width="22" height="16" viewBox="0 0 22 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M9.38599 4.35691C9.1543 4.21789 8.86573 4.21425 8.6306 4.34738C8.39547 4.48051 8.25012 4.72983 8.25012 5.00003V11C8.25012 11.2702 8.39547 11.5196 8.6306 11.6527C8.86573 11.7858 9.1543 11.7822 9.38599 11.6432L14.386 8.64315C14.6119 8.50761 14.7501 8.26348 14.7501 8.00003C14.7501 7.73658 14.6119 7.49245 14.386 7.35691L9.38599 4.35691ZM12.5424 8.00003L9.75012 9.67539V6.32467L12.5424 8.00003Z" fill="#FBF8EF"/>
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M16.0308 0.641311C12.6821 0.379329 9.31816 0.379328 5.96951 0.641311L3.72881 0.816613C2.36624 0.923214 1.25155 1.94442 1.02632 3.29246C0.505551 6.40924 0.505551 9.59082 1.02632 12.7076C1.25155 14.0556 2.36624 15.0768 3.72881 15.1834L5.96951 15.3587C9.31815 15.6207 12.6821 15.6207 16.0308 15.3587L18.2715 15.1834C19.6341 15.0768 20.7488 14.0556 20.974 12.7076C21.4948 9.59082 21.4948 6.40924 20.974 3.29246C20.7488 1.94442 19.6341 0.923214 18.2715 0.816613L16.0308 0.641311ZM6.0865 2.13674C9.35728 1.88085 12.643 1.88085 15.9138 2.13674L18.1545 2.31204C18.8301 2.3649 19.3828 2.87125 19.4945 3.53966C19.9879 6.49277 19.9879 9.50729 19.4945 12.4604C19.3828 13.1288 18.8301 13.6352 18.1545 13.688L15.9138 13.8633C12.643 14.1192 9.35727 14.1192 6.0865 13.8633L3.8458 13.688C3.17019 13.6352 2.61749 13.1288 2.50581 12.4604C2.01239 9.50729 2.01239 6.49277 2.50581 3.53966C2.61749 2.87125 3.17019 2.3649 3.8458 2.31204L6.0865 2.13674Z" fill="#FBF8EF"/>
                            </svg>

                        </a>
                        <!-- Linkedin -->
                        <a class = "text-decoration-none" href="#">
                            <svg width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M3 0.25C1.48122 0.25 0.25 1.48122 0.25 3C0.25 4.51878 1.48122 5.75 3 5.75C4.51878 5.75 5.75 4.51878 5.75 3C5.75 1.48122 4.51878 0.25 3 0.25ZM1.75 3C1.75 2.30964 2.30964 1.75 3 1.75C3.69036 1.75 4.25 2.30964 4.25 3C4.25 3.69036 3.69036 4.25 3 4.25C2.30964 4.25 1.75 3.69036 1.75 3Z" fill="#FBF8EF"/>
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M0.25 7C0.25 6.58579 0.585786 6.25 1 6.25H5C5.41421 6.25 5.75 6.58579 5.75 7V20C5.75 20.4142 5.41421 20.75 5 20.75H1C0.585786 20.75 0.25 20.4142 0.25 20V7ZM1.75 7.75V19.25H4.25V7.75H1.75Z" fill="#FBF8EF"/>
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M7.25 7C7.25 6.58579 7.58579 6.25 8 6.25H12C12.4142 6.25 12.75 6.58579 12.75 7V7.43402L13.1853 7.24748C13.9336 6.92676 14.7339 6.72565 15.5433 6.65207C18.3182 6.3998 20.75 8.58038 20.75 11.3802V20C20.75 20.4142 20.4142 20.75 20 20.75H16C15.5858 20.75 15.25 20.4142 15.25 20V13C15.25 12.6685 15.1183 12.3505 14.8839 12.1161C14.6495 11.8817 14.3315 11.75 14 11.75C13.6685 11.75 13.3505 11.8817 13.1161 12.1161C12.8817 12.3505 12.75 12.6685 12.75 13V20C12.75 20.4142 12.4142 20.75 12 20.75H8C7.58579 20.75 7.25 20.4142 7.25 20V7ZM8.75 7.75V19.25H11.25V13C11.25 12.2707 11.5397 11.5712 12.0555 11.0555C12.5712 10.5397 13.2707 10.25 14 10.25C14.7293 10.25 15.4288 10.5397 15.9445 11.0555C16.4603 11.5712 16.75 12.2707 16.75 13V19.25H19.25V11.3802C19.25 9.47587 17.589 7.97227 15.6791 8.14591C15.025 8.20536 14.3784 8.36807 13.7762 8.6262L12.2954 9.26079C12.0637 9.3601 11.7976 9.33634 11.5871 9.19756C11.3767 9.05878 11.25 8.82354 11.25 8.57143V7.75H8.75Z" fill="#FBF8EF"/>
                            </svg>
                        </a>
                        <!-- p -->
                        <a class = "text-decoration-none" href="#">
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M10 0.25C4.61522 0.25 0.25 4.61522 0.25 10C0.25 15.3848 4.61522 19.75 10 19.75C15.3848 19.75 19.75 15.3848 19.75 10C19.75 4.61522 15.3848 0.25 10 0.25ZM1.75 10C1.75 5.44365 5.44365 1.75 10 1.75C14.5563 1.75 18.25 5.44365 18.25 10C18.25 14.5563 14.5563 18.25 10 18.25C9.14272 18.25 8.31599 18.1192 7.53865 17.8766C7.96125 17.213 8.29734 16.498 8.53871 15.7491L9.05527 13.7719C9.26869 14.0765 9.55482 14.3229 9.88766 14.4888C10.2205 14.6547 10.5895 14.7348 10.9612 14.7219C13.4965 14.7219 15.2778 12.4181 15.2778 9.55626C15.2778 6.80719 13.0334 4.75282 10.1537 4.75282C6.56746 4.75282 4.66152 7.16344 4.66152 9.78188C4.66152 11.005 5.30871 12.5191 6.34777 13.0059C6.50214 13.0772 6.60308 13.0059 6.62683 12.8931C6.65057 12.7804 6.79292 12.211 6.85828 11.9495C6.86853 11.9081 6.8686 11.8643 6.85825 11.8229C6.8479 11.7815 6.82756 11.7433 6.79902 11.7116C6.40647 11.1585 6.19867 10.4957 6.20527 9.81751C6.20321 9.33072 6.29996 8.84858 6.48964 8.40026C6.67932 7.95195 6.958 7.54679 7.30881 7.2093C7.65962 6.87181 8.07527 6.60902 8.53058 6.43682C8.9859 6.26463 9.47143 6.18662 9.95777 6.20751C10.4133 6.18086 10.8694 6.24946 11.2969 6.40892C11.7245 6.56838 12.1141 6.8152 12.4409 7.13365C12.7677 7.45209 13.0246 7.83516 13.1951 8.25841C13.3656 8.68166 13.446 9.13581 13.4312 9.59188C13.4312 11.8363 12.2912 13.3978 10.8187 13.3978C10.6285 13.4124 10.4375 13.381 10.262 13.3062C10.0865 13.2313 9.93165 13.1153 9.81049 12.9679C9.68933 12.8206 9.60546 12.6462 9.56599 12.4595C9.52651 12.2729 9.53259 12.0794 9.58371 11.8956C9.6681 11.5454 9.77947 11.1847 9.888 10.8332C10.0849 10.1955 10.2725 9.58809 10.2725 9.12876C10.293 8.97956 10.2811 8.82769 10.2375 8.68353C10.1939 8.53938 10.1196 8.40636 10.0198 8.29358C9.91998 8.1808 9.79697 8.09093 9.65919 8.03011C9.52141 7.9693 9.37211 7.93899 9.22152 7.94126C8.39027 7.94126 7.71933 8.80813 7.71933 9.96001C7.71315 10.3671 7.78992 10.7711 7.94496 11.1475L6.96527 15.3038C6.84525 16.0891 6.84499 16.8882 6.96449 17.6736C6.31392 17.4161 5.70391 17.0782 5.14671 16.6722C4.16962 15.9584 3.35317 15.0348 2.76292 13.9593C2.53071 13.5362 2.33712 13.0962 2.18299 12.6446C1.90223 11.8144 1.75 10.925 1.75 10Z" fill="#FBF8EF"/>
                            </svg>
                        </a>
                            <!-- WhatsApp -->
                        <a class = "text-decoration-none" href="#">
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M6.88583 5.16985C7.06879 5.17475 7.27163 5.18465 7.46461 5.61303C7.59259 5.89821 7.80817 6.42321 7.98378 6.85087C8.12043 7.18366 8.23288 7.45751 8.26102 7.51356C8.32532 7.64156 8.3649 7.78926 8.28081 7.96156C8.2709 7.98188 8.26153 8.00132 8.25251 8.02003C8.18505 8.16003 8.13705 8.25966 8.02361 8.38994C7.97696 8.44348 7.92837 8.50219 7.87996 8.56069C7.79482 8.66358 7.71023 8.7658 7.63772 8.83802C7.50912 8.96593 7.37551 9.10391 7.5239 9.35991C7.67238 9.6159 8.19179 10.4579 8.95855 11.1373C9.78288 11.8712 10.4997 12.1805 10.862 12.3368C10.9324 12.3672 10.9894 12.3918 11.0312 12.4126C11.2885 12.5406 11.4418 12.5209 11.5902 12.3486C11.7387 12.1762 12.2333 11.6001 12.4065 11.3441C12.5747 11.0881 12.7478 11.1275 12.9853 11.2161C13.2227 11.3047 14.4891 11.9251 14.7463 12.0531C14.7971 12.0784 14.8447 12.1012 14.8887 12.1224C15.0677 12.2082 15.1894 12.2665 15.2409 12.3535C15.3052 12.4618 15.3052 12.9739 15.0926 13.5746C14.8749 14.1752 13.8262 14.7513 13.3513 14.7956C13.3063 14.7999 13.2616 14.8053 13.2155 14.8108C12.7803 14.8635 12.2278 14.9303 10.2595 14.1555C7.83412 13.2018 6.2331 10.8354 5.90941 10.357C5.88386 10.3192 5.86626 10.2932 5.85685 10.2806L5.85138 10.2733C5.70411 10.0762 4.80316 8.87067 4.80316 7.62685C4.80316 6.43682 5.38938 5.81726 5.65677 5.53467C5.67372 5.51676 5.68938 5.50021 5.70354 5.48494C5.94094 5.22895 6.21802 5.16495 6.39113 5.16495C6.56433 5.16495 6.73744 5.16495 6.88583 5.16985Z" fill="#FBF8EF"/>
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M0.184058 19.3314C0.102241 19.6284 0.37273 19.9025 0.670778 19.8247L5.27812 18.6213C6.73247 19.409 8.36988 19.8275 10.037 19.8275H10.0419C15.5279 19.8275 19.9999 15.3815 19.9999 9.91626C19.9999 7.26735 18.9659 4.77594 17.0862 2.90491C15.2064 1.03397 12.7082 0 10.0419 0C4.55595 0 0.083985 4.44605 0.083985 9.91136C0.0833616 11.65 0.54228 13.3582 1.41467 14.8645L0.184058 19.3314ZM2.86079 15.2629C2.96761 14.8752 2.91425 14.4608 2.71269 14.1127C1.97253 12.8348 1.58346 11.3855 1.58399 9.91136C1.58399 5.28158 5.37725 1.5 10.0419 1.5C12.3118 1.5 14.4294 2.37698 16.028 3.96805C17.6247 5.55737 18.4999 7.66611 18.4999 9.91626C18.4999 14.5459 14.7066 18.3275 10.0419 18.3275H10.037C8.62049 18.3275 7.22851 17.9718 5.99254 17.3023C5.65802 17.1211 5.26714 17.0738 4.89904 17.17L2.13664 17.8915L2.86079 15.2629Z" fill="#FBF8EF"/>
                            </svg>
                        </a>
                        <!-- Facebook -->
                        <a class = "text-decoration-none" href="#">
                            <svg width="12" height="20" viewBox="0 0 12 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M4.48769 1.78769C5.47225 0.803123 6.80761 0.25 8.2 0.25H10.9C11.3142 0.25 11.65 0.585786 11.65 1V4.6C11.65 5.01421 11.3142 5.35 10.9 5.35H8.2C8.16022 5.35 8.12206 5.3658 8.09393 5.39393C8.0658 5.42206 8.05 5.46022 8.05 5.5V7.45H10.9C11.131 7.45 11.349 7.5564 11.4912 7.73844C11.6333 7.92048 11.6836 8.15785 11.6276 8.3819L10.7276 11.9819C10.6441 12.3158 10.3442 12.55 10 12.55H8.05V19C8.05 19.4142 7.71421 19.75 7.3 19.75H3.7C3.28579 19.75 2.95 19.4142 2.95 19V12.55H1C0.585786 12.55 0.25 12.2142 0.25 11.8V8.2C0.25 7.78579 0.585786 7.45 1 7.45H2.95V5.5C2.95 4.10761 3.50312 2.77226 4.48769 1.78769ZM8.2 1.75C7.20544 1.75 6.25161 2.14509 5.54835 2.84835C4.84509 3.55161 4.45 4.50544 4.45 5.5V8.2C4.45 8.61421 4.11421 8.95 3.7 8.95H1.75V11.05H3.7C4.11421 11.05 4.45 11.3858 4.45 11.8V18.25H6.55V11.8C6.55 11.3858 6.88579 11.05 7.3 11.05H9.41442L9.93942 8.95H7.3C6.88579 8.95 6.55 8.61421 6.55 8.2V5.5C6.55 5.06239 6.72384 4.64271 7.03327 4.33327C7.34271 4.02384 7.76239 3.85 8.2 3.85H10.15V1.75H8.2Z" fill="#FBF8EF"/>
                            </svg>
                        </a>
                        <!-- Twitter -->
                        <a class = "text-decoration-none" href="#">
                            <svg width="12" height="20" viewBox="0 0 12 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M4.50711 0.251668C4.52375 0.250556 4.54043 0.25 4.55711 0.25H7.10288C7.51709 0.25 7.85288 0.585786 7.85288 1V4.71382H10.5902C11.0044 4.71382 11.3402 5.04961 11.3402 5.46382V8.07934C11.3402 8.49355 11.0044 8.82934 10.5902 8.82934H7.85288V13.5109C7.85288 13.5482 7.85009 13.5855 7.84454 13.6225C7.82688 13.7399 7.8364 13.8597 7.87238 13.9729C7.90835 14.086 7.96981 14.1894 8.05203 14.275C8.13425 14.3607 8.23503 14.4263 8.34661 14.4669C8.45819 14.5074 8.57759 14.5218 8.69561 14.509C8.71422 14.507 8.7329 14.5056 8.75161 14.505C9.1471 14.4917 9.53699 14.4075 9.9027 14.2563C10.0955 14.1766 10.3127 14.1808 10.5023 14.2679C10.6919 14.355 10.8366 14.5171 10.9017 14.7153L11.7125 17.1826C11.7851 17.4036 11.7511 17.6457 11.6204 17.8382C11.4495 18.0898 11.1888 18.2794 10.9571 18.4165C10.7082 18.5638 10.4139 18.6961 10.1007 18.807C9.47921 19.0271 8.71883 19.1859 7.98083 19.1868C5.88452 19.219 4.36585 18.4612 3.37895 17.362C2.41035 16.2833 1.99368 14.9206 1.99368 13.7812V8.82934H1C0.585786 8.82934 0.25 8.49355 0.25 8.07934V5.82999C0.25 5.51813 0.442993 5.2388 0.734695 5.12848C1.56531 4.81435 2.2866 4.26537 2.81065 3.54846C3.33255 2.83449 3.63637 1.98469 3.68551 1.10183C3.69243 0.889088 3.77622 0.685631 3.92193 0.529516C4.07461 0.365927 4.28384 0.266586 4.50711 0.251668ZM5.1252 1.75C4.97987 2.71769 4.60246 3.63904 4.02161 4.43365C3.42988 5.24316 2.64874 5.8914 1.75 6.32373V7.32934H2.74368C3.15789 7.32934 3.49368 7.66512 3.49368 8.07934V13.7812C3.49368 14.586 3.79628 15.5816 4.49507 16.3599C5.17463 17.1167 6.26954 17.7141 7.96265 17.6869L7.97472 17.6867C8.51641 17.6867 9.10925 17.5669 9.59985 17.3931C9.80899 17.319 9.98752 17.2394 10.1272 17.1631L9.70778 15.8868C9.41943 15.9525 9.12508 15.9916 8.82846 16.0032C8.49186 16.0354 8.15217 15.9922 7.83415 15.8766C7.50681 15.7576 7.21116 15.5651 6.96995 15.3138C6.72875 15.0626 6.54843 14.7593 6.4429 14.4274C6.34359 14.1151 6.31299 13.7852 6.35288 13.4604V8.07934C6.35288 7.66512 6.68866 7.32934 7.10288 7.32934H9.84024V6.21382H7.10288C6.68866 6.21382 6.35288 5.87803 6.35288 5.46382V1.75H5.1252Z" fill="#FBF8EF"/>
                            </svg>
                        </a>
                    </div>
                </section>
                <!-- Section: Form -->

                    <!-- Section: Links -->
                    <section class="footer-form p-5">
                        <!--Grid row-->
                        <div class="row">
                            <!--Grid column-->
                            <div class="col-lg-2 col-md-6 mb-4 mb-md-0 text-start">
                                <h5 class="text-uppercase mb-4">Info</h5>

                                <ul class="list-unstyled mb-0">
                                    <li>
                                        <a href="#!" class="text-uppercase text-white text-decoration-none">About Us</a>
                                    </li>
                                    <li>
                                        <a href="#!" class="text-uppercase text-white text-decoration-none">PRIVACY POLICY</a>
                                    </li>
                                    <li>
                                        <a href="#!" class="text-uppercase text-white text-decoration-none">TERMS & CONDITIONS</a>
                                    </li>
                                    <li>
                                        <a href="#!" class="text-uppercase text-white text-decoration-none">Faq</a>
                                    </li>
                                </ul>
                                <h5 class="text-uppercase mt-4 ">SUPPORT</h5>
                                <h5 class="text-uppercase ">ACCOUNT</h5>
                                <h5 class="text-uppercase ">REPRESENTATIVES</h5>
                            </div>
                            <!--Grid column-->

                            <!--Grid column-->
                            <div class="col-lg-4 col-md-6 mb-4 mb-md-0 text-start">
                                <h5 class="text-uppercase mb-4">PRODUCTS & SERVICES</h5>

                                <ul class="list-unstyled mb-0">
                                    <li>
                                        <a href="#!" class="text-uppercase text-white text-decoration-none">About Us</a>
                                    </li>
                                    <li>
                                        <a href="#!" class="text-uppercase text-white text-decoration-none">PRIVACY POLICY</a>
                                    </li>
                                    <li>
                                        <a href="#!" class="text-uppercase text-white text-decoration-none">TERMS & CONDITIONS</a>
                                    </li>
                                    <li>
                                        <a href="#!" class="text-uppercase text-white text-decoration-none">Faq</a>
                                    </li>
                                </ul>
                            </div>
                            <!--Grid column-->

                            <!--Grid column-->
                            <div class="col-lg-3 col-md-6 mb-4 mb-md-0">
                                <div class = "text-start">
                                    <h4 class="text-uppercase">SUPPORT</h4>
                                    <h4 class="text-uppercase">ACCOUNT</h4>
                                    <h4 class="text-uppercase">REPRESENTATIVES</h4>
                                </div>
                            </div>

                            <!--Grid column-->
                            <div class="col-lg-3 col-md-6 mb-4 mb-md-0 text-start">
                                <section class="">
                                    <form action="">
                                        <!--Grid row-->
                                        <div class="justify-content-center">
                                            <!--Grid column-->
                                            <div class="col-auto">
                                                <p class="pt-2 mb-1">
                                                    <strong>Sign up to be a part of Ultra family</strong>
                                                </p>
                                            </div>
                                            <!--Grid column-->

                                            <!--Grid column-->
                                            <div class="col-md-12 col-6 ">
                                                <!-- Email input -->
                                                <div class="form-outline mb-1">
                                                    <label class="form-label" for="form5Example2">Receive newsletter and exclusive member discounts</label>
                                                    <input type="email" id="form5Example2" class="form-control" placeholder="email" />
                                                </div>
                                            </div>
                                            <!--Grid column-->

                                            <!--Grid column-->
                                            <div class="col-auto">

                                                <!-- Submit button -->
                                                <button type="submit" class="btn btn-light">
                                                    Subscribe
                                                </button>
                                            </div>
                                            <!--Grid column-->
                                        </div>
                                        <!--Grid row-->
                                    </form>
                                </section>
                            </div>
                            <!--Grid column-->
                        </div>
                        <!--Grid row-->
                    </section>
                </div>
                <!-- Section: Links -->

            </div>
            <!-- Copyright -->

        </footer>
        <!-- Footer -->
    </div>
</div>
</body>
</html>
<style>

    @media (min-width: 500px) {
        .flex {
            display:block;
        }
    }
    .flex{
        display:flex;
    }
    .blue-header{
        background: rgba(255, 255, 255, 0.6);
        backdrop-filter: blur(22px);
        z-index: 999;
    }
    .footer-social{
        width:40%;
        background: #C4D62D;
        border-radius: 20px 20px 0px 0px;
    }
    .footer-form{
        background: #353E6C;
        color:#fff;
    }
</style>
